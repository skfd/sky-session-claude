...
- **Standby (N)** → the walking-away button, and the window's copy of `SessionCli standby` (below). Remote Control is per process, so a project with nothing running is a project your phone cannot open and cannot start one in either; this leaves a `claude rc` host in every repo you have worked in this week, so the phone shows your projects instead of an empty list. The count is what it would open under the filters the list is showing. Clicking states the plan first — the projects, when each was last worked in, and the ones passed over with the reason — and the dialog is what `--yes` is on the command line: nothing it touches can lose work, but a host per project is worth being told about before they land. They land in *one* Windows Terminal window, a tab named for each repo — sixteen console windows is a desktop you cannot use afterwards, and one window is also the stop lever a host otherwise lacks: close a tab to end that host, close the window to end them all. Without Windows Terminal on the machine it falls back to the window per project it used to open. It shares its gate with **Restart stale**: one sweep at a time, since they share the one status line.
...
- **Take a mark back** (**Ctrl+Z**, redo with **Ctrl+Y**) → the marks are the one thing in the window the scanner cannot put back for you, and pressing the key again only helps while you still know which card it was. Ctrl+Z walks the last marks back one keystroke at a time, wherever the cards have got to since — which is usually out of sight, because the mark is what hid them. A keystroke over a selection is one step, and it comes back the way it went: press D over a selection where one was already done, one crossed out and one untouched, and the undo gives you those three again rather than clearing all three. The history is this sitting at the window and does not survive closing it, and it restores what *you* saw — a mark an agent made in between is overwritten, because that is what taking your own keystroke back means.
...
SessionCli list --stale             # live, but behind the installed build (hosts too)
SessionCli list --hosts             # every Remote Control host, behind or not
...
`standby` is the verb for walking away from the desk. Remote Control is per process, so a project with nothing running is a project your phone cannot open — and there is no way to start one *from* the phone either. That asymmetry is the whole reason this verb exists: everything else here can wait until you are back, and this one has to happen before you leave. It reads recency off the transcripts and leaves a Remote Control host running in each project you have worked in lately, so the phone shows a list of your repos instead of an empty one.

**A host, not a session** — the difference matters and the two wear nearly the same words. `claude --remote-control` is one interactive session in a terminal that happens to be bridged; `claude rc` is a server for a folder, which spawns a session there each time the phone asks for one — and, the way standby starts it, not before: left to itself a host pre-creates an empty session on arrival, under a name nobody chose, and a sweep of twenty projects is twenty of those in the phone's list. Standby starts the second kind, because the phone is where second thoughts happen: a session per project caps you at one conversation per repo, and starting another is precisely the thing a phone cannot do for itself. The costs are real and worth knowing — a host has no terminal you can walk up to and type in, and the conversations it spawns are `sdk-cli`, the kind `close` refuses to sweep. Reaching one *particular* existing conversation is still `resume <id>`, and a plain bridged session in one folder is still `new --in <path>`.

Every session a host spawns is named after its project — `sky-session-claude-wondrous-seal` — because standby passes the repo as `--remote-control-session-name-prefix`. Left to itself the prefix is the machine's hostname and every row on the phone reads `cc-pc-sorted-stallman`, which tells you nothing about which repo you are about to type into. That flag is the whole difference between a usable phone list and an unusable one.

Like the other sweeps it prints its plan and does nothing until `--yes` — not because anything is at risk (everything it touches is something it just made) but because the plan is also the count of hosts about to start. They open as tabs in a single Windows Terminal window called `sky-standby`, one `wt` invocation for the whole run: sixteen invocations would each find no window by that name, each make one, and leave you with four. Tomorrow's standby joins the same window if it is still up, and closing a tab is how you stop one host — the one gesture the `rc` skill otherwise has to say does not exist. `--since` sets how far back "lately" reaches, written the way you say it (`7d`, `12h`, `90m`; a week by default), and `--recent <n>` caps how many come up.

What it declines to open, it reports. A repo a host is already serving is passed over rather than doubled up, and finding those is the subtle half: **a host publishes no session of its own** — only the conversations it spawns do, and those come and go, so a served folder with nothing open looks exactly like an empty one in the session registry. Nor does it leave a file: the `bridge-pointer.json` Claude Code writes beside a project's transcripts is written for a session, and a host that has spawned none has written nothing. What proves a host is the host itself — a running `claude rc` process whose working directory is the folder, read out of the process the way its command line already was. On this machine three of the twenty-two hosts up had no pointer under them; all twenty-two had a working directory.

A live host is the *only* thing that counts, and the registry gets no say in either direction. It used to: any session with Remote Control connected passed its folder over, which sounds right and is not, because reaching a folder is not what standby is for. A repo held by a bridged `claude --remote-control` terminal shows exactly one conversation on the phone and no way to open a second — the state this verb exists to get you out of, read as a reason not to. Now a host opens beside it, and since the terminal is not in the way, the plan does not mention it either.

A repo that has since been deleted is reported rather than `cd`-ed into, because a shell that cannot reach a folder stays where it started. And two kinds of folder are not projects at all: the worktrees an agent makes under a repo's `.claude`, which are the newest folders on disk at exactly the moment they stop being folders, and anything without a `.git` — a home folder, the folder every repo sits under, wherever a terminal happened to be open when a question came up. Those look exactly like projects in a transcript, with the same recency and often more sessions than the real ones, and they cost a row each on a screen that is nothing but rows. All of it is reported with the reason rather than dropped quietly, and none of the rules apply to `--in <path>`, where you are pointing and pointing beats any rule about it.

One thing standby does not do is clean up after itself. `claude rc` will not ask about trust — it says to run `claude` in the folder once and stops — so a folder Claude Code has never seen gets no host, which the plan says out loud rather than pretending otherwise. And at the other end of the day, `close --finished` cannot take a host away: hosts are invisible to it and their conversations are deliberately protected. Ending the hosts is still a thing you do by hand.
...
`list --stale` answers the same question without doing anything about it, and it counts the hosts in — they are the harnesses least able to say they have fallen behind. They come back in their own `Hosts` array rather than as rows among the sessions, because a host has no status, no context and no session id, and filling those in with blanks is the conflation the whole vocabulary exists to avoid. Each one carries how long it has been up, how many conversations it is answering for, the verdict the sweep would reach and why, and the line that puts it back. `--hosts` asks for them on their own, current ones included, and the two compose the way the words do — `--hosts` says to include them, `--stale` goes on narrowing, so both together is the ones behind. Without either flag there is no `Hosts` array at all: every other listing is about sessions, and the twice-daily dump the morning brief reads keeps meaning exactly what it did.
