...
What it declines to open, it reports. A repo a host is already serving is passed over rather than doubled up, and finding those is the subtle half: **a host publishes no session of its own** — only the conversations it spawns do, and those come and go, so a served folder with nothing open looks exactly like an empty one in the session registry. What proves it is `bridge-pointer.json`, the record a host leaves beside that project's transcripts for its own `--continue`. It outlives the process that wrote it, so the pointer is only believed when its pid is still a running Claude — on this machine two of the three pointers on disk were stale.
